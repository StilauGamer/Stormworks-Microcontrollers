...
LifeBoatAPI.LBTouchScreen = {
    ...
    lbtouchscreen_newStyledButton = function (this, x, y, width, height,
        ...
        ---@field toggled boolean text to display in the button
        ...
        local button = {
            ...
            ---@section lbstyledbutton_isToggled
            --- Checks for the user click and waiting for it to be unclicked.
            ---@param this LBTouchScreenButtonStyled
            lbstyledbutton_isToggled = function(this)
                if this.touchScreenRef.wasPressed then -- Checks if the user has clicked the button
                    this.toggled = not this.toggled -- Takes the toggled variable and changes the value to the opposite of itself by using NOT
                end
            end;
            ---@endsection
            ...
            lbstyledbutton_draw = function(this)
                (this:lbstyledbutton_isToggled() and this.fillPushColor or this.fillColor):lbcolorrgba_setColor() -- This runs if the user clicks on the button and there is a fillPushColor or fillcolor set. Then it sets the color from the lbcolorrgba.
                screen.drawRectF(this.x, this.y, this.width, this.height); -- Makes a filled rectangle with the x, y, width and height of the button.

                this:lbstyledbutton_isToggled() -- Calls the isToggled function.
                ;(this.toggled and this.textPushColor or this.textColor):lbcolorrgba_setColor() -- Checks if the button is toggled and if the textPushColor or textColor is set and sets the color by getting it from the lbcolorrgba.
                screen.drawTextBox(this.x+2, this.y+1, this.width-1, this.height-1, this.text, 0, 0); -- Makes a textBox with the x, y, widht, height and text from the button

                this.borderColor:lbcolorrgba_setColor() -- Takes the color from the borderColor variable and sets the color through lbcolorrgba
                screen.drawRect(this.x, this.y, this.width, this.height) -- makes a rectangle by using the x, y, width and height of the button.
                -- The lbcolorrgba sets the color using RGBa values.
            end;
            ---@endsection
        }
        return button
